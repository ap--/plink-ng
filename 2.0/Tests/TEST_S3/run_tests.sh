...
if [[ -n "${PLINK_TESTS_S3_SUPPORT:-}" ]]; then
    ...
    S3_PREFIX="$S3_BUCKET/plink_simulated"
    cd "$WORK"
    mkdir -p real_local
    # Reference computed locally from the same files, so nothing has to be
    # committed or regenerated by hand: this only checks that reading via S3
    # gives the same result as reading the identical bytes off local disk.
    for ext in pgen psam pvar bed bim fam; do
        aws s3 cp --no-sign-request --only-show-errors \
            "$S3_BUCKET/plink_simulated.$ext" "real_local/plink_simulated.$ext"
    done
    for fileset in pfile bfile; do
        "$PLINK2" "--$fileset" real_local/plink_simulated --geno-counts \
            --out "real_local_$fileset" --silent > /dev/null ||
            fail "real S3 --$fileset: local reference run exited nonzero"
        ...
        diff -q "real_local_$fileset.gcount" "real_$fileset.gcount" > /dev/null ||
